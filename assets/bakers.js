...
function loadTSV() {
  ...
  $.ajax({
    type: "GET",  
    url: "https://cdn.shopify.com/s/files/1/0554/6190/4593/files/backers.tsv?v=1708542431", 
    dataType: "text",       
    success: function(response){
      ...
      if(lists.length > 1) {
        for(var i = 1; i < lists.length; i ++) {
          ...
          // var r_num = Math.floor(Math.random() * (305 - 201 + 1)) + 201;
          var r_num = Math.floor(Math.random() * (705 - 601 + 1)) + 601;
          if(r_num == 0) {
            r_num = 1
          }
  ...
  $('[data-backer-search]').on('keydown', function(e) {
    clearTimeout(timer);
  ...
  function buildGrid() {
    ...
    var keyword = (($('[data-backer-search]').val()).toLowerCase()).replace(' ', '');
    ...
    var $container = $('[data-backer-container]');
    $container.html('');

    if(_count == 0) {
      $('[data-backer-container]').append('<span data-empty-result>No results.</span>');
    }
  ...
  function backerSort() {
    for(var i = 0; i < id_arr.length; i ++) {
      if(s_cat == 'id' || s_cat == null) {
        ...
        var $item = $('[data-backer-container] > .item [data-id="' + (id_arr[i].toLowerCase()).trim() + '"]').closest('.item');
        $item.css('order', i * direc);
      }
      else {
        var $item = $('[data-backer-container] > .item [data-name="' + (name_arr[i].replace('"', '').replace(/\s+/g, '')) + '"]').closest('.item');
        $item.css('order', i * direc);
      }
    }
  }
